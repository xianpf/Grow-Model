...
class GrowResNeXtBottleneck(nn.Module):
    def __init__(
        ...
        # 策略： embryo和adults采用不同的optimizer，embryo长得快，成熟期或掐表或判定。
        ...
        self.embryo_conv1 = Conv2d(in_channels, self.embryo_channels, 1, 2, bias=False)
        # TODO: specify init for self.conv1 1*1conv
        self.embryo_bn1 = norm_func(bottleneck_channels)

        self.embryo_conv2 = Conv2d(self.embryo_channels, self.embryo_channels, 3, 1, padding=dilation, bias=False, dilation=dilation)
        # TODO: try to init with spercific rules, or just init once then never change.
        nn.init.kaiming_uniform_(self.conv2.weight, a=1)
        self.bn2 = norm_func(self.embryo_channels)

        self.conv3 = Conv2d(self.embryo_channels, out_channels, kernel_size=1, bias=False)
        ...
        # TODO: adults_channels need a large channel volume. say max embryo_channels*32 = 8*32=256
